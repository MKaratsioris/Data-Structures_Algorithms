...
@timeit
def binary_search_iterative(sorted_iterable: Iterables, target: Simple) -> tuple[Simple, int] | None:
    """
...
def binary_search_recursive(sorted_iterable: Iterables, item: Simple) -> Simple | None:
    """
    ...
    if guess == item:
        ...
        return binary_search_recursive(sorted_iterable=sorted_iterable[middle + 1:], item=item)
    elif guess > item:
        return binary_search_recursive(sorted_iterable=sorted_iterable[:middle - 1], item=item)
...
if __name__ == '__main__':
    ...
    test_target: int = 10
    index, steps = binary_search_iterative(test_list, test_target)
    ...
    index = binary_search_recursive(numbers, guess)
    print(f"\n{maximum=}\t{index=}\n")